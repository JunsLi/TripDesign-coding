...
function [Sd3,Ave3,Convergence_curve3]=SMA(N,Max_iter,lb,ub,dim,fobj,Run_no)
...
Convergence_curve3=zeros(1,Max_iter);
...
while  it <= Max_iter
    ...
    it=it+1;
    Convergence_curve3(it)=Destination_fitness;
    Ceqfit_run(irun)=Destination_fitness;
end
display(['Run no : ', num2str(irun)]);
display(['The best solution obtained by SMA is : ', num2str(bestPositions,10)]);
display(['The best optimal value of the objective funciton found by SMA is : ', num2str(Destination_fitness,10)]);
disp(sprintf('--------------------------------------'));
end
Ave3=mean(Ceqfit_run);
Sd3=std(Ceqfit_run);
